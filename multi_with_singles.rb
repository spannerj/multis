...
file = CSV.parse(File.read("sheets/" + MONTH + ".csv"), headers: true)
days = []
day = ''
day_list=[]
file.each do |row|
...
days.each { |day|
  ...
  when 3
    type = 'patent'
    wb = Bet::Calc.patent(w_arr, stake: 0.05)
    eb = Bet::Calc.patent(p_arr, stake: 0.05)
  when 4
    type = 'lucky15'
    wb = Bet::Calc.lucky15(w_arr, stake: 0.05)
    eb = Bet::Calc.lucky15(p_arr, stake: 0.05)
  when 5
    type = 'lucky31'
    wb = Bet::Calc.lucky31(w_arr, stake: 0.05)
    eb = Bet::Calc.lucky31(p_arr, stake: 0.05)
  when 6
    type = 'lucky63'
    wb = Bet::Calc.lucky63(w_arr, stake: 0.05)
    eb = Bet::Calc.lucky63(p_arr, stake: 0.05)
  when 7
    type = 'super-heinz'
    wb = Bet::Calc.super_heinz_with_singles(w_arr, stake: 0.05)
    eb = Bet::Calc.super_heinz_with_singles(p_arr, stake: 0.05)
  else
    result = {}
    result['date'] = day[0]['date']
    result['type'] = 'unknown'
...
CSV.open("results_with_singles_" + MONTH + ".csv", "wb") do |csv|
